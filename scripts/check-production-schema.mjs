...
const checks = [
  ...
  ['admin_tier', "SELECT code AS ok FROM subscription_tiers WHERE code = 'admin' AND location_retention_days = 2 AND max_children = 5 AND is_active = true"],
  ...
  ['parent_push_tokens', "SELECT to_regclass('public.parent_push_tokens') AS ok"],
